...
We now come to Phase 04. As the human overseeing this work, I judge this is the most tricky phase.

Read the master plan at `80-java-monorepo-add-01-remove-before-merge/dd-2989727-move-java-to-monorepo-plan.md  ` to understand the overall plan. I am working on completing Phase 04. Phases 00, 01, 02 and 05 are complete.

The standalone repo is in the folder `copilot-sdk-java-00`. The monorepo is in the folder `copilot-sdk-00`.

Us the `gh` CLI for any GitHub interaction.

The big ticket item in Phase 04 is completing the migration of the "reference implementation sync" methanism. I say "mechanism" because it is a system of workflows, agents, scripts, PRs, and human interaction. This system is working very well in the standalone repo, but has not yet been run in anger in the monorepo.

The one big change when moving the `reference-impl-sync` mechanism from the standalone repo to the monorepo is that it will no longer run on a schedule. It will only be run imperatively, at the explicit request from a human.

Read how the reference impl sync is implemented in the standalone repo.

Read the not-yet-complete work that has been done so far in copying it to the monorepo. Note that files have been renamed in keeping with the overall goal to keep Java separated where it makes sense to do so.

I already asked you to read the master plan above. Give special attention to Phase 04.

I'm going to give the job of implementing this to a separate `copilot --yolo`. You must write out the plan to `80-java-monorepo-add-01-remove-before-merge\dd-3007546-phase-04-plan.md`. I'll pass that to `copilot --yolo`

Please also write out `80-java-monorepo-add-01-remove-before-merge\dd-3007546-phase-04-grader.md` to be a separate prompt to grade the work of the implementer. I'll give that to `copilot --yolo` after it says it's done.
