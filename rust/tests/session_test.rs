...
#[tokio::test]
async fn create_session_rejects_cloud_config() {
    let (client, _server_read, _server_write) = make_client();

    let error = expect_sdk_error(
        client.create_session(cloud_session_config()).await,
        "cloud config should use create_cloud_session",
    );

    assert!(
        matches!(error, github_copilot_sdk::Error::InvalidConfig(ref message) if message.contains("create_cloud_session")),
...
async fn create_cloud_session_sends_cloud_create_without_session_id() {
    ...
    let create_handle = tokio::spawn({
        ...
        async move {
            client
                .create_cloud_session(cloud_session_config())
                .await
                .unwrap()
        }
    });
...
#[tokio::test]
async fn create_cloud_session_rejects_caller_session_id_and_provider() {
    let (client, _server_read, _server_write) = make_client();

    let error = expect_sdk_error(
        client
            .create_cloud_session(cloud_session_config().with_session_id("caller-id"))
            .await,
    ...
    let error = expect_sdk_error(
        client.create_cloud_session(config).await,
...
#[tokio::test]
async fn create_cloud_session_request_flags_follow_handlers() {
    struct InputHandler;
    #[async_trait]
...
#[tokio::test]
async fn create_cloud_session_buffers_early_notifications_until_session_id_is_registered() {
    let (client, server_read, server_write) = make_client();
    let mut server = FakeServer {
        read: server_read,
        write: server_write,
...
#[tokio::test]
async fn create_cloud_session_buffers_early_requests_until_session_id_is_registered() {
    struct InputHandler;
    #[async_trait]
    impl UserInputHandler for InputHandler {
        async fn handle(
